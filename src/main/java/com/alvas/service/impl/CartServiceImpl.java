...
public class CartServiceImpl implements CartService{
	...
	@Override
	public String addToCart(long userId, CartDto cartDto) {
		User user=userRepository.findById(userId).orElseThrow(()->new UserNotFoundException("User with id:"+userId+"not found"));
		List<Product> products=productRepository.findByProductNameIgnoreCaseIn(cartDto.getProductDtos().stream().map(ProductDto::getProductName).toList());
		if(products.size()<cartDto.getProductDtos().size())
			throw new ProductNotFoundException("product not found");
		Cart cart=new Cart();
		cart.setUser(user);
		cart.setTotalQuantity(cartDto.getProductDtos().stream().mapToInt(ProductDto::getQuantity).sum());
		
		cartDto.getProductDtos().forEach(productDto->{
